$(function() {
	govern_addlist_affair();
});
//各种事件绑定
function govern_addlist_affair() {
	$("#username").blur(function() {
		govern_addclick_username();
	});
	$(".password").blur(function() {
		govern_addclick_password();
	});
	$("#email").blur(function() {
		govern_addclick_email();
	});
	$("#name").blur(function() {
		govern_addclick_name();
	});
	$("#phone").blur(function() {
		govern_addclick_phone();
	});
//	$("#remark").blur(function() {
//		govern_addclick_remark();
//	});
	
};
//表单提交验证
function govern_add_click() {
	
	if (!govern_addclick_username()) {
		return false;
	} else if (!govern_addclick_password()) {
		//		alert(2);
		return false;
	} else if (!govern_addclick_name()) {
		//		alert(3);
		return false;
	} else if (!govern_addclick_phone()) {
		//		alert(5);
		return false;
	} else if (!govern_addclick_email()) {
		//		alert(4);
		return false;
	} else {
		return true;
...
//验证管理员账号
var falg = false;
function govern_addclick_username() {
...
//		$("#butSel").removeAttr("disabled");
		var urlpath=$(".path").val()+"/govern/ajaxusername";
		$.ajax( {
			type : "post",
			url :urlpath,
			dataType : "json",
...
//验证管理员密码
function govern_addclick_password() {
...
//验证管理员邮箱
function govern_addclick_email() {
	var email = $("#email").val();
	if(email == ""){
...
//		return false;
	...
	if (!result) {
		...
		var urlpath=$(".path").val()+"/govern/ajaxEmail";
		$.ajax( {
			type : "post",
			url :urlpath,
			dataType : "json",
...
//验证手机号
function govern_addclick_phone() {
	var phone = $("#phone").val();
	if(phone == ""){
	...
	if (!result) {
		...
		var urlpath=$(".path").val()+"/govern/ajaxPhone";
		$.ajax( {
			type : "post",
			url :urlpath,
			dataType : "json",
...
//验证管理员备注
function govern_addclick_remark() {
